# -*- coding: utf-8 -*-
# i18n_pkg/lang_ru.py (fixed)
from .meta import BASE_STRINGS


STRINGS = {
    **BASE_STRINGS,
    "Configuration": "Настройки",
    "Sample folder": "Папка с сэмплами",
    "Select a folder containing 1.wav, 2.wav, ...": "Выберите папку с 1.wav, 2.wav, ...",
    "Browse…": "Обзор…",
    "WorkspaceHeadline": "Chromatic Scale Generator PLUS!",
    "WorkspaceTagline": "Создавайте аккуратные хроматические шкалы с пошаговой настройкой.",
    "SampleSetup": "Настройка сэмплов",
    "SampleSetupIntro": "Укажите папку с пронумерованными сэмплами и задайте параметры генерации.",
    "TimingAndTuning": "Тайминг и строй",
    "ProcessingOptions": "Параметры обработки",
    "GenerationStatus": "Состояние генерации",
    "SummaryReady": "Готово: {semitones} полутонов от {note}{octave}, пауза {gap:.2f} с.",
    "SummaryWaiting": "Выберите корректную папку, чтобы начать генерацию.",
    "&File": "&Файл",
    "Open Sample Folder…": "Открыть папку сэмплов…",
    "Starting note": "Начальная нота",
    "Starting octave": "Начальная октава",
    "Semitone range": "Количество полутонов",
    "Gap (seconds)": "Пауза (секунды)",
    "Apply pitch transformation": "Изменять высоту",
    "Dump individual pitched samples": "Сохранять отдельные семплы с тоном",
    "Randomize sample selection": "Случайный выбор семплов",
    "Peak normalize each sample (pre-pitch)": "Нормализация по пику (до изменения тона)",
    "Embed FL Studio Slicex slice markers": "Встраивать метки срезов FL Studio Slicex",
    "Generate Chromatic": "Собрать хроматику",
    "Cancel": "Отмена",
    "Open Output Folder": "Открыть папку вывода",
    "Logs will appear here…": "Логи будут здесь...",
    "GenerationProgress": "Ход генерации",
    "GenerationSummary": "Сводка генерации",
    "LogOutput": "Журнал генерации",
    "LayoutResetMessage": "Макет сброшен к настройкам по умолчанию.",
    "SettingsTabLanguage": "Язык",
    "SettingsTabPaths": "Пути",
    "SettingsTabAdvanced": "Дополнительно",
    "LanguageLabel": "Язык",
    "AutoReloadTranslations": "Автоматически перезагружать переводы при изменении текста",
    "OutputFolder": "Папка вывода",
    "OutputFolderPlaceholder": "Оставьте пустым, чтобы использовать папку сэмплов",
    "EnableVerboseLogging": "Включить подробное логирование",
    "ResetWindowLayout": "Сбросить расположение окна",
    "Select Output Folder": "Выбрать папку вывода",
    "StartingNoteHint": "Первая нота, с которой строится хроматическая шкала.",
    "StartingOctaveHint": "Октава, применяемая к начальной ноте.",
    "SemitoneRangeHint": "Общее число полутонов в создаваемой шкале.",
    "GapSecondsHint": "Пауза между нотами в секундах.",
    "Credits": "Благодарности",
    "Wiki": "Вики",
    "Tutorial": "Учебник",
    "Settings": "Настройки",
    "&Help": "&Справка",

    "Folder not found.": "Папка не найдена.",
    "Need at least 1.wav in this folder.": "В папке нужен хотя бы 1.wav.",
    "No sequential samples found (1.wav, 2.wav, ...).": "Нет последовательных файлов (1.wav, 2.wav, ...).",
    "Select Sample Folder": "Выбрать папку сэмплов",
    "Selected folder: {p}": "Выбрана папка: {p}",
    "No output file yet.": "Выходного файла ещё нет.",
    "Invalid folder. Place '1.wav', '2.wav', ... in it.": "Неверная папка. Поместите '1.wav', '2.wav', ...",
    "'{name}' already exists. Overwrite?": "'{name}' уже существует. Перезаписать?",
    "Cancelled by user.": "Отменено пользователем.",
    "Generating…": "Создание…",
    "Cancelling…": "Отмена…",
    "Done! Saved to {p}": "Готово! Сохранено в {p}",
    "An error occurred.": "Произошла ошибка.",
    "⚠️ Generation was cancelled.": "⚠️ Создание отменено.",

    "Footer": "Оригинал: @ChillSpaceIRL - Ремастер: @nullfreq_ и Malloy  |  Версия {version} - {month_name} {year}",
    "Join Discord": "Вступить в Discord",
    "CreditsText": "Chromatic Scale Generator Plus Remastered\n\nОригинал: @ChillSpaceIRL\nРемастер: @nullfreq_ и Malloy\nВерсия {version} - {month_name} {year}\n\nСпасибо за использование!",

    "Found {n} sample(s) (1.wav..{m}.wav).": "Найдено {n} сэмплов (1.wav..{m}.wav).",
    "Semitones: {s} | Gap: {g:.3f}s | Pitched: {p}": "Полутонов: {s} | Пауза: {g:.3f}с | Изменение тона: {p}",
    "Peak normalization: ON": "Нормализация по пику: ВКЛ",
    "[{a}/{b}] Loading {name}": "[{a}/{b}] Загрузка {name}",
    "normalized": "нормализовано",
    "Concatenating chromatic scale...": "Склеивание хроматики...",
    "Embedding FL Studio Slicex slice markers...": "Встраивание меток срезов FL Studio Slicex...",
    "Saved: {path}": "Сохранено: {path}",
    "Dumped {n} pitched sample(s) to: {dir}": "Сохранено {n} семпл(ов) с тоном в: {dir}",
}
